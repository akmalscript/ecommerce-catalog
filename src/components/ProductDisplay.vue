        ...
        <div v-else class="container" :class="!isProductAvailable ? 'bg-gray' : product.data.category === 'men\'s clothing' ? 'bg-blue' : 'bg-pink'">
            ...
            <div class="card">
                <div v-if="!isProductAvailable" class="product-unavailable-container">
                    <div class="sad-face-overlay">
                        <img src="../assets/sad-face.svg" alt="sad face">
                    </div>
                    <div class="product-details unavailable-details">
                        <p class="unavailable-text">This product is unavailable to show</p>
                        <div class="cta">
                            <button type="button" @click="getSingleProduct()" class="cta-next border-dark">Next product</button>
                        </div>
                    </div>
                </div>
                ...
                <div v-else class="product-container">
                    ...
                    <div class="product-details">
                        <div class="top">
                            ...
                            <div class="sub-title">
                                <span :class="product.data.category === 'men\'s clothing' ? 'font-navy' : 'font-magenta'">{{ product.data.category }}</span>
                                <div class="rating">
                                    <span>{{ product.data.rating.rate }}/5</span>
                                    <div class="rating-dots">
                                        <span v-for="n in filledDots" :key="'filled-'+n" :class="product.data.category === 'men\'s clothing' ? 'bg-navy' : 'bg-magenta'" class="circle"></span>
                                        <span v-for="n in emptyDots" :key="'empty-'+n" :class="product.data.category === 'men\'s clothing' ? 'border-navy' : 'border-magenta'" class="circle empty"></span>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    computed: {
        filledDots() {
            if (!this.product.data || !this.product.data.rating) return 0;
            return Math.round(this.product.data.rating.rate);
        },
        emptyDots() {
            return 5 - this.filledDots;
        }
    },
    methods: {
        async callAPI() {
